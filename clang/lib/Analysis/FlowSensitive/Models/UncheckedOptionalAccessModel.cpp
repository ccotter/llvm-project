...
static bool hasNestedNamespace(const NamespaceDecl &NS,
                               llvm::StringRef NestedName,
                               llvm::StringRef TopName) {
  if (NS.getDeclName().isIdentifier() && NS.getName() == NestedName &&
      NS.getParent() != nullptr) {
    const auto *NextNS = dyn_cast_or_null<NamespaceDecl>(NS.getParent());
    return NextNS && NextNS->getDeclName().isIdentifier() &&
           NextNS->getName() == TopName && NextNS->getParent() != nullptr &&
           NextNS->getParent()->isTranslationUnit();
  }
  return false;
}
...
static bool hasOptionalClassName(const CXXRecordDecl &RD) {
  ...
  if (RD.getName() == "optional") {
    if (const auto *N = dyn_cast_or_null<NamespaceDecl>(RD.getDeclContext()))
      return N->isStdNamespace() || isTopLevelNamespaceWithName(*N, "absl") ||
             isTopLevelNamespaceWithName(*N, "bsl");
    return false;
  }
  ...
  if (RD.getName() == "NullableValue") {
    const auto *N = dyn_cast_or_null<NamespaceDecl>(RD.getDeclContext());
    return N != nullptr && hasNamespace(*N, "bdlb", "BloombergLP");
  }
