...
public:
  ...
  std::unordered_map<const clang::DeclRefExpr *, CopiedFromData>
      d_variablesCopiedFrom;
  std::unordered_multimap<const clang::VarDecl *, const clang::DeclRefExpr *>
      d_varUsages;
  ...
  const decltype(d_varUsages) &varUsages() const { return d_varUsages; }
  const decltype(d_scopes) &scopes() const { return d_scopes; }
  const decltype(d_escaped) &escaped() const { return d_escaped; }
